...
const SearchPanel = ({ onSelectLocation, userLocation }) => {
    ...
    useEffect(() => {
        ...
        const locationsWithDistance = Object.entries(locationData)
        ...
        setNearbyPlaces(locationsWithDistance);
    }, [userLocation]);

    // Filter locations based on search and category
    useEffect(() => {
        let filtered = Object.entries(locationData)
            .filter(([id, loc]) => loc.type === 'destination');

        if (selectedCategory !== 'all') {
            filtered = filtered.filter(([id, loc]) => loc.category === selectedCategory);
        }
